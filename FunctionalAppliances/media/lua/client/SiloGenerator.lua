...
FASiloGenerator.doBuildMenu = function(player, menu, square, SiloGenerator)
	...
	if SiloGenerator:getPipedFuelAmount() < 500 then
		...
		--menu:addOption(FA.translation.addfuel,
		--		  nil,	
		--		  FASiloGenerator.onUseSiloGenerator,
		--		  getSpecificPlayer(player),
		--		  SiloGenerator,
	...
	--	menu:addOption(FA.translation.takefuel,
	--			  nil,	
	--			  FASiloGenerator.onUseSiloGenerator,
	--			  getSpecificPlayer(player),
	--			  SiloGenerator,
...
FASiloGenerator.onUseSiloGenerator = function(junk, player, SiloGenerator, NewSiloGenerator, fuelCan, MyChoice)
	...
	--	local emptyCan = inv:FindAndReturn("Base.EmptyPetrolCan")
	--	if fuelCan ~= nil then
	--		ISTimedActionQueue.add(SiloGeneratorTakeFuel:new(player, SiloGenerator, emptyCan, 100))
	--	else
	--		player:Say(FA.translation.sayneedemptyfuelcan)
	--	end
	--	return
	...
	for index,t in ipairs(FA.SiloGeneratorList) do
		...
		if square ~= nil then 
			...
			local objects = square:getObjects()
				...
				--if SiloGenerator and NewSiloGenerator then
					--print("have both silo and generator")
