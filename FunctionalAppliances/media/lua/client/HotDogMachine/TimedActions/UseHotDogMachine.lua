...
local function addXP(machine, foodType)
	...
	if foodType == "Sausage" then
		Sausage:setCooked(true);
	elseif foodType == "ViennaSausage" then
		VSausage:setCooked(true);
	elseif foodType == "FrankfurterSausage"	then
		FSausage:setCooked(true);
	end	
		

	--removed 10-30-2023 due to bug with disappearing items in containers
	--if machine:getContainer() then
	--	machine:getContainer():AddItem("FunctionalAppliances.FAHotdog")
	--else
	--	inv:AddItem("FunctionalAppliances.FAHotdog")
	--end
