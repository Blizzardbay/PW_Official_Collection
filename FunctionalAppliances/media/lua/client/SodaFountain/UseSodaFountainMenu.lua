		...
		for i=1,square:getObjects():size() do
			...
			if thisObject:getSprite() then
				...
				if groupName == "Tabletop Soda" then				
					...
					if properties:Is(IsoFlagType.waterPiped) == false then
...
function MachineNoWater(player)
	player:Say("This machine needs more water.")
end

function MachineNoSyrup(player)
	player:Say("This dispenser is not connected to a syrup box.")
end

function MachineEmptySyrup(player)
	player:Say("This dispenser is connected to an empty syrup box.")
end
...
UseSodaFountainMenu.walkToFront = function(thisPlayer, SodaFountainObject)
	...
	if not controllerSquare then
		controllerSquare = SodaFountainObject:getSquare()
	end
...
UseSodaFountainMenu.onUseSodaFountain = function(worldobjects, player, machine, soundFile, flavorChoice, foundSyrup, foundCO2)
	if UseSodaFountainMenu.walkToFront(player, machine) then
		...
		elseif not (SandboxVars.WaterShutModifier > -1 and GameTime:getInstance():getNightsSurvived() < SandboxVars.WaterShutModifier and square:isOutside() == false) and (machine:getSprite():getProperties():Is(IsoFlagType.waterPiped) == false or machine:getWaterAmount() < changeWaterAmount) then
			MachineNoWater(player)
		elseif flavorChoice == "Soda" and (vanillaFountainCup or FAFountainCup) then
