...
VFEContext.inventoryMenu = function(playerid, context, items)
	...
	for _, v in ipairs(items) do
		...
		if instanceof(item, "HandWeapon") then
			...
			-- Sling
			if item:isTwoHandWeapon() or item:hasTag("AllowSling") then
				blacklisted = false
	...
	if enabled then
		if bayonetFound then
			if bayonetBlocked == nil then
				...
				listEntry.notAvailable = true
				tooltip.description = tooltip.description .. getText("IGUI_ContextMenu_BayonetBlockedArg", bayonetBlocked:getDisplayName())
			end
		else
			listEntry.notAvailable = true
...
function VFEContext:AltOperation(item, target, player, context, altOperation, enabled)
	...
	local listEntry = context:addOption(actionString, item, VFEAltOperationContext.callAction, altOperation, target, player)
	
	local tooltip = ISInventoryPaneContextMenu.addToolTip();
	actionString = getText(altOperation.OperationText[target].. "Arg", item:getDisplayName())
	tooltip:setName(tostring(actionString .. "\n"))
	tooltip.texture = getTexture("media/textures/item_" .. ScriptManager.instance:getItem(altOperation.Name[target]):getIcon() .. ".png")
	if not enabled then
		listEntry.notAvailable = true
		tooltip.description = tooltip.description .. getText("IGUI_ContextMenu_MoveToInv")
	else
