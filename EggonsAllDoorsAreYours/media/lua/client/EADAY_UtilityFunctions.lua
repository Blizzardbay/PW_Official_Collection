EADAY = EADAY or {}
local Settings = SandboxVars.eggonsAllDoorsAreYours
...
function EADAY.isInstallationValid(doorConfig)
    ...
    if carpentryLevel < Settings.MinimumCarpentryLevel then
...
function EADAY.adjustDoorWeight(possiblyDoor) 
    local weightMultiplier = SandboxVars.eggonsAllDoorsAreYours.DoorsWeightMultiplier
    if EADAY.isValidEADAYDoorItem(possiblyDoor) and weightMultiplier ~= 1 then
        local baseDoorWeight = possiblyDoor:getWeight()
        local modifiedWeight = Math.ceil(baseDoorWeight * weightMultiplier)
        possiblyDoor:setWeight(modifiedWeight)        
        possiblyDoor:setActualWeight(modifiedWeight) 
    end
end