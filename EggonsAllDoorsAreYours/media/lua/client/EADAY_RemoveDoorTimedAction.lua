...
function RemoveDoor:perform()
    ...
    local carpentryLevelAboveMin = math.max(0, carpentryLevel - settings.MinimumCarpentryLevel)

    -- min carpentry = 4 starts at 60%
    local successProbability = math.min(1, (settings.SuccessChance / 100) + carpentryLevelAboveMin * (settings.SuccessChanceIncrease / 100))
    -- print("CHANCE: ", successProbability)
    ...
    if removalSuccess.success then
        ...
        EADAY.adjustDoorWeight(doorItem) 
